const express=require('express'),cors=require('cors'),{Pool}=require('pg'),jwt=require('jsonwebtoken'),{Vonage}=require('@vonage/server-sdk');
...
async function migrate(){
  ...
  try{
    await c.query(`
      ...
      CREATE TABLE IF NOT EXISTS drivers(id SERIAL PRIMARY KEY,phone VARCHAR(20) UNIQUE NOT NULL,name VARCHAR(100) NOT NULL,vehicle_type VARCHAR(50) DEFAULT 'تكتك مكشوف',plate VARCHAR(30),status VARCHAR(20) DEFAULT 'offline',rating NUMERIC(3,2) DEFAULT 5.00,wallet_balance INTEGER DEFAULT 0,wallet_id VARCHAR(20) UNIQUE,zone_id INTEGER REFERENCES zones(id),docs_verified BOOLEAN DEFAULT false,is_active BOOLEAN DEFAULT true,created_at TIMESTAMPTZ DEFAULT NOW());
...
// HEALTH
app.get('/',(req,res)=>res.json({success:true,message:'تكتك 🛺',version:'3.0.0'}));
app.get('/health',async(req,res)=>{try{await pool.query('SELECT 1');res.json({success:true,db:'connected',server:'running'});}catch(e){res.status(500).json({success:false,db:'disconnected'});}});

// AUTH
app.post('/api/auth/send-otp',async(req,res)=>{
  try{
    const{phone}=req.body;
    if(!phone||!iraqPhone(phone))return res.status(400).json({success:false,error:'أرسل رقم عراقي يبدأ بـ +9647'});
    const otp=genOTP();
...
app.post('/api/auth/verify-otp',async(req,res)=>{
  try{
    ...
    if(!s)return res.status(400).json({success:false,error:'لا يوجد كود لهذا الرقم'});
...
app.post('/api/trips/:id/accept',auth,driverOnly,async(req,res)=>{
  try{
    ...
    if((w.rows[0]?.balance||0)<0)return res.status(400).json({success:false,error:'رصيدك سالب، شحن المحفظة'});
...
app.post('/api/driver/status',auth,driverOnly,async(req,res)=>{
  try{
    ...
    if(!['online','offline'].includes(status))return res.status(400).json({success:false,error:'online أو offline فقط'});
...
app.get('/api/drivers/available',async(req,res)=>{try{const r=await pool.query(`SELECT id,name,vehicle_type,rating,zone_id FROM drivers WHERE status='online' AND is_active=true ORDER BY rating DESC`);res.json({success:true,data:r.rows});}catch(e){err500(res,e,'فشل السائقين');}});
